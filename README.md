...
- Node.jsの標準ライブラリのみで動作\
  Node.jsの利用できる環境で、ファイル配置したら即実行可能としたかったので。\
  （パッケージのインストールやコンパイルなどは不要。
...
```sh
node --env-file=.env src/rllm.ts > review.md
```
